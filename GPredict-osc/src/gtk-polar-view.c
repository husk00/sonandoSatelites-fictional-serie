...
update_sat    (gpointer key, gpointer value, gpointer data)
{
    ...
    *catnum = sat->tle.catnr;
	...
	lo_address t = lo_address_new(NULL, "9999");
	...
	name = (char*) malloc(strlen(sat->nickname)+1); /* space for new name */
	strcpy(name, sat->nickname); /* copy it */
	...
	/* path */
	path = (char*) malloc(strlen("/gpredict/sats/")+strlen(name)+1);
	strcpy(path, "/gpredict/sats/");
	strcat(path, name);

	float doppler = -100.0e06 * (sat->range_rate / 299792.4580); // Hz

	/* send message */
	if (lo_send(t, path, "i", 0) == -1)
